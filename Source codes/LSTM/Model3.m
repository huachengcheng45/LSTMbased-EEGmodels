%% Model3 2维CNN电极-频段分开+lstm算法
imageInputSize = [30 5 1]; % input size
numClasses = 1; % num of classes, because this model is regression model, this parameter is 1
filterSize1 = [30,1]; % kernel size of CNN layer
filterSize2 = [1,5]; % kernel size of CNN layer
numFilters1 = 30; % kernel number of CNN layer
numFilters2 = 30; % kernel number of CNN layer
numHiddenUnits1=70; % num of units in hidden layer1
numHiddenUnits2=70; % num of units in hidden layer2
numHiddenUnits3=70; % num of units in hidden layer3
layers = [
    sequenceInputLayer(imageInputSize,'Normalization','none','Name','Input')
    sequenceFoldingLayer('Name','fold')
    batchNormalizationLayer('Name','bn1')
    convolution2dLayer(filterSize1,numFilters1,'Name','conv1')% 1D-CNN layer for electrodes
    reluLayer('Name','relu1')
    convolution2dLayer(filterSize2,numFilters2,'Name','conv2')% 1D-CNN layer for frequency bands
    reluLayer('Name','relu2')
    sequenceUnfoldingLayer('Name','unfold')
    flattenLayer('Name','flatten')

    % 3-layer LSTM
    lstmLayer(numHiddenUnits1, 'OutputMode', 'sequence','Name','bilstm1')
    dropoutLayer(0.25,'Name','drop1')
    lstmLayer(numHiddenUnits2, 'OutputMode', 'sequence','Name','bilstm2')
    dropoutLayer(0.25,'Name','drop2')
    lstmLayer(numHiddenUnits3, 'OutputMode', 'last','Name','bilstm3')
...
for i=1:10
    ...
    %adjusting data set for the training of LSTM or BiLSTM and adding an
    %additional axis for two dimensional CNN
    XTrain=reshape(XTrain,[size(XTrain,1),size(XTrain,2),1,size(XTrain,3),size(XTrain,4)]);
    XVal=reshape(XVal,[size(XVal,1),size(XVal,2),1,size(XVal,3),size(XVal,4)]);
    XTest=reshape(XTest,[size(XTest,1),size(XTest,2),1,size(XTest,3),size(XTest,4)]);
    
    XTrain=squeeze(mat2cell(XTrain,size(XTrain,1),size(XTrain,2),1,size(XTrain,4),ones(1,size(XTrain,5))));
    XVal=squeeze(mat2cell(XVal,size(XVal,1),size(XVal,2),1,size(XVal,4),ones(1,size(XVal,5))));
    XTest=squeeze(mat2cell(XTest,size(XTest,1),size(XTest,2),1,size(XTest,4),ones(1,size(XTest,5))));
...
% Save the results
save cv_m3_3lstm70_result net info loss all_ytest all_ypred indices
